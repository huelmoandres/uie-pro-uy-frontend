...
* **Carpetas Clave:**
    ...
    * `src/utils/`: Formateadores de fecha (date-fns + date-fns-tz) y normalizadores de IUE.

## 2. Zona Horaria (Regla Estricta)
* **APP_TIMEZONE:** Todas las fechas de decretos, movimientos y expedientes son en **hora Uruguay (America/Montevideo)**. La app solo opera en Uruguay.
* **Constante:** Usar `APP_TIMEZONE` de `@constants/timezone` para formateo de fechas.
* **Formatters:** `formatDate`, `formatDateShort`, `getYearInAppTimezone` usan `formatInTimeZone` con America/Montevideo.

## 3. Estándares de Codificación e Idioma
...
## 4. Gestión de Datos y Red (Axios + TanStack Query)
...
## 5. Estilos y Modo Oscuro (NativeWind)
...
## 7. Performance Senior (React Native)
...
## 9. 🚨 Sistema de Alertas y Confirmaciones (Custom UI)
...
## 10. 🎨 Estética y Sistema de Diseño (Senior Pro UI/UX)
...
## 11. 🏗️ Arquitectura de Datos y Formularios (Zod + Hook Form + Query)
...
## 12. 🧩 Componentes de Dominio y Lógica de Reutilización
...
## 15. 🔄 Gestión de Actualizaciones OTA (Expo Updates)
