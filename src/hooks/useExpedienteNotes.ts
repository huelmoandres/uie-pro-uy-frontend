...
const followQueryKey = (iue: string, userId: string | null) =>
  ["expediente-follow", iue, userId] as const;
...
export function useExpedienteNotes(iue: string) {
   ...
   * Incluye userId en la query key para evitar mostrar notas de otro usuario al cambiar de cuenta.
   */
  const { data: followData } = useQuery({
    queryKey: followQueryKey(iue, userId),
  ...
  const mutation = useMutation({
    mutationFn: (newNotes: string | null) =>
      ExpedienteService.updateNotes(iue, newNotes),
