...
export const Paginator: React.FC<PaginatorProps> = React.memo(({
    ...
    const validCurrentPage = Math.max(1, Math.min(currentPage || 1, safeTotalPages));
    const safeTotalItems = totalItems || 0;

    const startIndex = safeTotalItems === 0 ? 0 : (validCurrentPage - 1) * pageSize + 1;
                ...
                <View className="flex-row items-center gap-1">
                    {pageSizeOptions.map((size) => (
                        <Pressable
                            ...
                            className={`min-w-[32px] h-8 items-center justify-center rounded-lg border ${pageSize === size
                        ...
                        >
                            <Text
                                className={`text-[12px] font-sans-bold ${pageSize === size ? 'text-white' : 'text-slate-500 dark:text-slate-400'
                                    }`}
                            >
                                {size}
                            </Text>
            ...
            {safeTotalPages > 1 && (
                <View className="flex-row items-center justify-between bg-slate-50 border border-slate-100 dark:bg-white/5 dark:border-white/10 rounded-2xl p-1">
                    ...
                    <Text className="text-[12px] font-sans-semi text-slate-500 px-2">
                        Pág <Text className="font-sans-bold text-slate-800 dark:text-white">{validCurrentPage}</Text> de {safeTotalPages}
                    </Text>
