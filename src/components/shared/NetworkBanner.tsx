...
export function NetworkBanner() {
    ...
    useEffect(() => {
        ...
        if (!isOnline) {
            ...
            translateY.value = withTiming(0, { duration: ANIM_MS, easing: Easing.out(Easing.ease) });
        } else {
            if (mode.current === 'offline') {
                ...
                reconnectedTimer.current = setTimeout(() => {
                    ...
                    translateY.value = withDelay(
                        100,
                        withTiming(-80, { duration: ANIM_MS, easing: Easing.in(Easing.ease) }, () => {
    ...
    return (
        <Animated.View style={[styles.container, animStyle, { paddingTop: topPadding }]}>
            <View style={[styles.banner, isOffline ? styles.offlineBanner : styles.onlineBanner]}>
                {isOffline ? (
