            ...
            <View style={styles.overlay}>
                ...
                <View className="w-full h-[95%] rounded-t-[32px] bg-white dark:bg-[#0B1120] border border-b-0 border-slate-100 dark:border-white/5 overflow-hidden flex-col">
                    ...
                    <View className="flex-row items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5">
                        ...
                        <Pressable onPress={onClose} className="h-8 w-8 items-center justify-center rounded-full bg-slate-100 dark:bg-white/5 active:opacity-70">
                            <X size={15} color="#94A3B8" />
                        </Pressable>
                    </View>

                    {/* Scrollable Form */}
                    <ScrollView className="px-6 py-5 flex-1" showsVerticalScrollIndicator={false}>

                        {/* Order & OrderBy */}
                        <Text className="text-[12px] font-sans-bold uppercase tracking-wider text-slate-400 mb-3">Ordenación</Text>
                        <View className="flex-row flex-wrap gap-2 mb-6">
                            {(
                                ...
                                <Pressable
                                    ...
                                    className={`px-3 py-2 rounded-xl border flex-row items-center gap-1 ${draft.orderBy === opt.value
                                ...
                                >
                                    <Text className={`text-[13px] font-sans-semi ${draft.orderBy === opt.value ? 'text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                                        {opt.label}
                                    </Text>
                                </Pressable>
                            ))}
                        ...
                        <Text className="text-[12px] font-sans-bold uppercase tracking-wider text-slate-400 mb-3">Dirección</Text>
                        <View className="flex-row gap-2 mb-6">
                            <Pressable
                                onPress={() => updateDraft('order', 'desc')}
                                className={`flex-1 flex-row items-center justify-center py-3 rounded-xl border ${draft.order === 'desc'
                                    ? 'bg-primary border-primary dark:bg-accent dark:border-accent'
                                    : 'bg-slate-50 border-slate-200 dark:bg-white/5 dark:border-white/10'
                                    }`}
                            >
                                <ArrowDownUp size={14} color={draft.order === 'desc' ? '#FFFFFF' : '#94A3B8'} />
                                <Text className={`ml-2 text-[13px] font-sans-semi ${draft.order === 'desc' ? 'text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                                    Descendente
                                </Text>
                            </Pressable>
                            <Pressable
                                onPress={() => updateDraft('order', 'asc')}
                                className={`flex-1 flex-row items-center justify-center py-3 rounded-xl border ${draft.order === 'asc'
                                    ? 'bg-primary border-primary dark:bg-accent dark:border-accent'
                                    : 'bg-slate-50 border-slate-200 dark:bg-white/5 dark:border-white/10'
                                    }`}
                            >
                                <ArrowDownUp size={14} color={draft.order === 'asc' ? '#FFFFFF' : '#94A3B8'} style={{ transform: [{ scaleY: -1 }] }} />
                                <Text className={`ml-2 text-[13px] font-sans-semi ${draft.order === 'asc' ? 'text-white' : 'text-slate-600 dark:text-slate-400'}`}>
                                    Ascendente
                                </Text>
                            </Pressable>
                        </View>
                        ...
                        {/* Sede */}
                        <Text className="text-[12px] font-sans-bold uppercase tracking-wider text-slate-400 mb-3">Sede Judicial</Text>
                        <TextInput
                    ...
                    <View className="flex-row items-center justify-end px-6 py-4 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/5">
                        <Pressable onPress={handleClear} className="px-5 py-3 active:opacity-70">
                            <Text className="text-sm font-sans-semi text-slate-500">Limpiar</Text>
                        </Pressable>
                        <Pressable
                            onPress={handleApply}
                            className="px-6 py-3 ml-2 rounded-xl bg-primary dark:bg-accent active:opacity-80 disabled:opacity-50"
                        >
                            <Text className="text-sm font-sans-bold text-white dark:text-primary">Aplicar Filtros</Text>
                        </Pressable>
                    </View>
                </View>
            </View>
