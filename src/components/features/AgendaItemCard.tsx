...
import { Clock, AlertTriangle, CheckCircle, XCircle, ChevronRight } from 'lucide-react-native';
...
function getUrgencyConfig(daysRemaining: number, status: IAgendaItem['status']) {
...
export const AgendaItemCard = React.memo(({ item }: Props) => {
        ...
        >
            ...
            <View className="flex-row items-start justify-between gap-3">
                ...
                <View className="items-end gap-1.5">
                    <View className={`flex-row items-center gap-1 rounded-full px-2.5 py-1 shrink-0 ${config.badge}`}>
            ...
            <View className="mt-3 rounded-xl bg-white/60 dark:bg-white/5 px-3 py-2">
                <Text className="text-[11px] font-sans text-slate-500 dark:text-slate-400 leading-relaxed" numberOfLines={2}>
                    {item.detectedText}
                </Text>
            </View>
